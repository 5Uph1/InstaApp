<x-app-layout>
    <x-slot name="header">
        <h2 class="font-extrabold text-2xl text-slate-900 tracking-tight">Edit Post</h2>
    </x-slot>
        ...
        <form action="{{ route('posts.update', $post) }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                <textarea name="caption" rows="4"
                    class="w-full border-slate-200 bg-slate-50/50 rounded-2xl p-3.5 text-sm focus:bg-white focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition duration-150">{{ old('caption', $post->caption) }}</textarea>
                @error('caption')
            ...
            @if ($post->image)
                <div>
                    ...
                    <div
                        class="relative w-44 h-44 rounded-2xl overflow-hidden border border-slate-200 shadow-sm p-1 bg-white">
            ...
            <div>
                <label class="block text-sm font-bold text-slate-700 mb-2">Ganti Gambar <span
                ...
                <p class="text-[11px] text-slate-400 mt-2">Biarkan kosong jika tidak ingin mengubah gambar.</p>
                @error('image')
                    <p class="text-rose-500 text-xs mt-1.5 font-semibold">{{ $message }}</p>
                @enderror
            </div>
